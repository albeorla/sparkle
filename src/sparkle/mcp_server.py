...
def build_app() -> FastMCP:
    ...
    def investigate(node_ref: str) -> str:
        ...
        return (
            ...
            "2. Gather specific, citable evidence — not opinion.\n"
            ...
            "weakens it. Put sources in `citations`.\n\n"
            f"{_vocabulary_block()}"
        )
    ...
    def synthesize(claim_ref: str) -> str:
        ...
        return (
            ...
            "2. JUDGE: weigh the support against the objections. You may only rule "
            "with `sparkle_rule` if the claim has a recorded objection — ruling a "
            "claim the adversary never attacked is refused by the server. If the "
            "claim is settled, pass `settle=true` to write the binding `ratified` "
            "version.\n"
