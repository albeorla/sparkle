...
- **Mutation tools** that enforce debate rules the raw CLI doesn't — for example, the judge's "rule" move refuses to ratify a claim the adversary never attacked. Because the MCP server is an *agent-driven* front-end, its rule move also enforces the author-distinct floor the autonomous harness uses: a claim only counts as challenged if the objection comes from a *different author* than the claim, so a single agent driving Sparkle over MCP cannot ratify its own claim off an objection it wrote against itself. The trusted human `sparkle rule` CLI path stays permissive (a human is trusted to challenge their own claim honestly).
